...
var bleach = Fn2(([color3, opacity = 1]) => {
  const base = color3;
  const lum = luminance2(base.rgb);
  const blend = vec32(lum);
  const L = min2(1, max2(0, float2(10).mul(lum.sub(0.45))));
  const result1 = blend.mul(base.rgb).mul(2);
  const result2 = float2(2).mul(blend.oneMinus()).mul(base.rgb.oneMinus()).oneMinus();
  const newColor = mix2(result1, result2, L);
  const A2 = base.a.mul(opacity);
  const mixRGB = A2.mul(newColor.rgb);
  mixRGB.addAssign(base.rgb.mul(A2.oneMinus()));
  return vec42(mixRGB, base.a);
});
export {
  bleach
};
...
three/build/three.tsl.js:
   ...
   *)
*/
//# sourceMappingURL=three_examples_jsm_tsl_display_BleachBypass__js.js.map
